...
class TextBlockNetPostProcessor(RegionNetPostProcessor):
    """
    A RegionNetPostProcessor used for processing the output of an image segmentation algorithm that is specialized on
    detecting text blocks (TextRegions) in an image.
    """
    ...
    def post_process(self, net_output):
        """
        Post process the raw net output ``net_output`` by removing the "other" channel and by removing too small
        connected components (i.e., that have a size smaller than one percent of the total number of pixels in the
        image).
        :param net_output: The raw net output resulting from applying the TensorFlow graph to the original image.
        :return: Post processed net output.
        """
    ...
    def to_polygons(self, net_output):
        """
        Converts the (post-processed) net output ``net_output`` to polygons.
        :param net_output: The (post-processed) net output.
        :return:
        """
